...
class OTPInputViewAutoFilled extends react_1.Component {
    constructor(props) {
        super(props);
        this.fields = [];
        ...
        this.bringUpKeyBoardIfNeeded = () => {
            ...
            const filledCount = digits.filter((d) => d && d !== '').length;
            const focusIndex = Math.min(filledCount, pinCount - 1);
            if (autoFocusOnLoad) {
        ...
        this.checkPinCodeFromClipBoard = async () => {
            if (!clipboard_1.default)
                return;
            const { pinCount = 6, onCodeFilled } = this.props;
        ...
        this.renderOneInputField = (_, index) => {
            const { codeInputFieldStyle, codeInputHighlightStyle, filledInputFieldStyle, errorInputFieldStyle, secureTextEntry, editable, keyboardType, selectionColor, keyboardAppearance, clearInputs, placeholderCharacter, placeholderTextColor, size = 'medium', inputWidth, inputHeight, fontSize, borderRadius, error, } = this.props;
            ...
            return (<react_native_1.View pointerEvents="none" key={`${index}view`} testID="inputSlotView">
                <react_native_1.TextInput testID="textInput" underlineColorAndroid="rgba(0,0,0,0)" style={inputStyle} ref={(ref) => {
                    ...
                    this.handleKeyPressTextInput(index, key);
                }} onFocus={() => this.handleFocus(index)} onBlur={() => this.handleBlur(index)} value={clearInputs ? '' : digits[index]} keyboardAppearance={keyboardAppearance} contextMenuHidden={true} keyboardType={keyboardType} textContentType="none" key={index} selectionColor={selectionColor} secureTextEntry={secureTextEntry} editable={editable} placeholder={placeholderCharacter} placeholderTextColor={placeholderTextColor}/>
            </react_native_1.View>);
        };
    ...
    render() {
        ...
        return (<react_native_1.View testID="OTPInputView" style={[mainContainerStyle, containerStyle]}>
                ...
                if (clearInputs) {
                    ...
                    const nextIndex = filledPinCount >= pinCount ? pinCount - 1 : filledPinCount;
                    this.focusField(nextIndex);
                }
            }}>
...
OTPInputViewAutoFilled.defaultProps = {
    pinCount: 6,
...
exports.default = OTPInputViewAutoFilled;
//# sourceMappingURL=OTPInputViewAutoFilled.js.map